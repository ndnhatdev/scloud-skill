---
name: rust-master
description: Deep Rust language and production guidance for ownership, borrowing, lifetimes, diagnostics, idioms, performance, tooling, smart pointers, traits, concurrency, Cargo, testing, unsafe, and API design. Use when Codex needs to explain Rust rules, choose between Box/Rc/Arc/RefCell/Mutex/RwLock, generics vs impl Trait vs dyn Trait, Result vs panic, async vs threads, interpret borrow-checker and type-system compiler errors, review Rust code for correctness, idiom, and performance, or build a focused Rust debugging, optimization, or study plan.
---
